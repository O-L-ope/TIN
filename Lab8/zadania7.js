...
function fillTable(auta) {
    const kontener = document.getElementById("cardiv");
    let tabelkaHtml = `
        <table border="1" id="tabelaAut">
            <tr>
                <th>Rok</th>
                <th>Przebieg</th>
    ...
    tabelkaHtml += `</table>`;
    kontener.innerHTML += tabelkaHtml;
}
fillTable(tablica);
...
let s2 = new Student("Jędrzej", "Bonaparte");
s2.oceny = new Ocena("AM", 2);
s2.oceny = new Ocena("ALG", 4.5);
s2.oceny = new Ocena("MAD", 3.5);
console.log(s2.hello());
console.log(s2.oceny);

const tablicaStudentow = [];
tablicaStudentow.push(s1, s2);

function displayStudents(studentData) {
    const belkaStudentow = document.getElementById('tablica-studentow');
    belkaStudentow.innerHTML = "";

    studentData.forEach((studentDane, sID) => {
        let gradesList = "<ul>";
        studentDane.tablicaOcen.forEach(lista => {
            gradesList += `
            <li>${lista.przedmiot}: 
            ${lista.wartosc}</li>
            `;
        ...
        const studentBlock = `
            <div class="student-main">
                <div class="student-personal" id="studentID-${sID}">
                    ${studentDane.imie} ${studentDane.nazwisko}
                </div>
                
                <div class="studentdata hidden" id="zawartosc-${sID}">
                    ${gradesList}
                    <div>
                        Średnia: ${studentDane.srednia.toFixed(2)}
                    </div>
                </div>
            </div>`;

        belkaStudentow.innerHTML += studentBlock;
    });

    studentData.forEach((_, sID) => {
        const header = document.getElementById(`studentID-${sID}`);
        const content = document.getElementById(`zawartosc-${sID}`);

        header.addEventListener('click', () => {
            // content.classList.toggle('active');
            content.classList.toggle('hidden');
            header.style.backgroundColor = content.classList.contains('hidden') ? "#9be3f9" : "#fbb087";
        });
    });
}
displayStudents(tablicaStudentow);